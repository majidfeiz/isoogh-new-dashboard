...
const GradeList = () => {
  ...
  const fetchData = useCallback(
    async (page = 1, currentFilters = {}, currentSort = sort) => {
      ...
      try {
        ...
        console.error("خطا در دریافت پایه‌ها", e);
        setData([]);
  ...
  const handleDelete = useCallback(
    async (id) => {
      const confirmed = window.confirm("آیا از حذف این پایه مطمئن هستید؟");
      if (!confirmed) return;
      ...
      try {
        ...
        console.error("خطا در حذف پایه", e);
      } finally {
        setLoading(false);
      }
    },
    ...
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          if (value === 1 || value === "1") return "فعال";
          if (value === 0 || value === "0") return "غیرفعال";
          return value ?? "-";
        },
      },
      {
  ...
  const handleSortingChange = useCallback(
    (nextSorting) => {
      const allowed = ["id", "name", "sort", "created_at"];
  ...
  return (
    <div className="page-content">
      <div className="container-fluid">
        <Breadcrumbs title="مدیریت پایه‌ها" breadcrumbItem="پایه‌ها" />
          ...
          <Col lg={12}>
            <Card>
              <CardHeader className="d-flex flex-wrap align-items-center justify-content-between gap-2">
                <div>
                  <h4 className="card-title mb-1">پایه‌ها</h4>
                  <p className="text-muted mb-0">
                    جستجو، صفحه‌بندی و مدیریت پایه‌های تحصیلی
                  </p>
                </div>
