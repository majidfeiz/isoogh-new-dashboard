...
export default function BaleLogsPage() {
  ...
  </Row></CardHeader><CardBody>{loading ? <div className="text-center py-5"><Spinner /></div> : error ? <div className="alert alert-danger">{error} <Button size="sm" onClick={load}>تلاش مجدد</Button></div> : !items.length ? <div className="text-center text-muted py-5">لاگی با این فیلترها یافت نشد.</div> : <div className="table-responsive"><table className="table table-bordered table-hover align-middle"><thead><tr><th>زمان</th><th>مجموعه</th><th>کاربر</th><th>Bale ID</th><th>رویداد</th><th>عملیات</th><th>Provider</th><th>وضعیت</th><th>Subject</th><th>Correlation ID</th><th>خطا</th></tr></thead><tbody>{items.map((item, index) => <tr key={item.id ?? index} role="button" tabIndex="0" onClick={() => setSelected(item)} onKeyDown={(event) => event.key === "Enter" && setSelected(item)}><td>{faDate(item.createdAt)}</td><td>{shown(item.schoolName ?? item.schoolId)}</td><td>{shown(item.userName ?? item.userId)}</td><td>{String(shown(item.baleUserId))}</td><td>{shown(item.event)}</td><td>{shown(item.operation)}</td><td>{shown(item.provider)}</td><td><Badge color={item.status === "success" ? "success" : item.status === "failed" ? "danger" : "warning"}>{shown(item.status)}</Badge></td><td>{shown(item.subjectType)}{item.subjectId != null && <small className="d-block">#{item.subjectId}</small>}</td><td>{String(shown(item.correlationId))}</td><td>{String(shown(item.errorMessage))}</td></tr>)}</tbody></table></div>}{!loading && !error && items.length > 0 && <Paginations perPageData={meta.limit} data={items} totalRecords={meta.total} currentPage={meta.page} setCurrentPage={(page) => change("page", String(page), false)} isShowingPageLength />}</CardBody></Card>
