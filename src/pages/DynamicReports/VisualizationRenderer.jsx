...
const VisualizationRenderer = ({ visualization, allowed = [] }) => {
  ...
  if (type === "kpi") return <Row className="g-3 mb-3">{chart.keys.map((key) => <Col md={4} key={key}><Card className="border"><CardBody><div className="text-muted">{key}</div><div className="display-6">{formatValue(chart.data[0]?.values?.[key], "decimal")}</div></CardBody></Card></Col>)}</Row>;
