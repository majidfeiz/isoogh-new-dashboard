// src/pages/Voip/Analytics/components/NullCallGroupSection.jsx
...
const NullCallGroupSection = ({ data, loading, error, exportParams, canExport }) => {
  ...
  const handleExport = async (schoolId) => {
    ...
    try {
      ...
      await downloadVoipAnalyticsCsv(
        API_ROUTES.voipAnalytics.exportNullCallGroup,
        params,
        `null-call-group${schoolId ? `-${schoolId}` : ""}.csv`
      )
    } catch {
  ...
  const countField = "null_call_group_count"
  const pctField = "null_call_group_percent"
        ...
        <div className="d-flex align-items-center justify-content-between gap-2">
          <div className="d-flex align-items-start gap-2 flex-grow-1">
            <div
              ...
              style={{ width: 32, height: 32, backgroundColor: "#556ee6", flexShrink: 0, marginTop: 2 }}
            >
              <i className="mdi mdi-link-variant-off fs-6" />
            </div>
            <div>
              <h6 className="mb-1 fw-semibold">تماس‌های بدون شناسه سیموتل</h6>
              <p className="mb-0 text-muted" style={{ fontSize: "0.8rem" }}>
                هنگام برقراری تماس، سیموتل یک شناسه منحصربه‌فرد (call_group_id) برمی‌گرداند.
                اگر این مقدار null باشد، سیستم نمی‌تواند تماس را بعداً در لاگ‌های سیموتل پیدا کند.
              </p>
            </div>
          </div>
          {canExport && (
                ...
                {barData.length === 0 ? <EmptyState /> : (
                  <ResponsiveContainer width="100%" height={240}>
                    <BarChart data={barData} margin={{ top: 5, right: 20, left: 0, bottom: 40 }}>
                      ...
                      />
                      <Bar dataKey="count" name="شناسه نامعتبر" fill="#556ee6" radius={[3, 3, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </Col>
...
export default NullCallGroupSection
