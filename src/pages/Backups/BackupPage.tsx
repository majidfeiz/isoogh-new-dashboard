      ...
      <Col xl="5"><Card className="h-100"><CardHeader><h5 className="mb-0">تنظیمات بک‌آپ</h5></CardHeader><CardBody>
        ...
        <div className="mt-3">
          {selectedSections.map((section) => <div className="border rounded p-3 mb-2" key={`date-${section}`}>
            ...
            <Row className="g-2"><Col sm="6"><DatePicker calendar={persian} locale={persianFa} value={dateDrafts[section].from || null} onChange={(value) => changeDate(section, "from", value)} format="YYYY/MM/DD" inputClass="form-control" className="backup-date-picker-popup" calendarPosition="bottom-right" portal portalTarget={document.body} fixRelativePosition placeholder="از تاریخ" disabled={running || busy} /></Col><Col sm="6"><DatePicker calendar={persian} locale={persianFa} value={dateDrafts[section].to || null} onChange={(value) => changeDate(section, "to", value)} format="YYYY/MM/DD" inputClass="form-control" className="backup-date-picker-popup" calendarPosition="bottom-right" portal portalTarget={document.body} fixRelativePosition placeholder="تا پایان روز" disabled={running || busy} /></Col></Row>
