...
- Parent-tag user import is job-based: `POST /parent-tags/import` accepts `file` plus `schoolId`, then `ParentTagList.jsx` persists the returned active `logId`/`schoolId` in session storage and polls the server-paginated `/parent-tags/imports/:logId` row log until a terminal status.
