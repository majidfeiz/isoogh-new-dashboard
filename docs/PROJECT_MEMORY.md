...
- Local backups stream CSV reports and recordings directly to a user-selected File System Access directory. Report responses are raw authenticated `text/csv` Web Streams written chunk-by-chunk without Blob/ArrayBuffer buffering; filenames come from safe `Content-Disposition` values with fixed fallbacks, report states/bytes are tracked independently in the manifest, and completed reports resume without directory scans. `backupController.ts` owns the abortable single-file queue, cross-tab lock, ACK ordering/retry (network/503 with `Retry-After`), chunk-level UI progress, temporary-file finalization, and manifest; directory handles are persisted in IndexedDB by backup id. The service normalizes camelCase/snake_case job resources. Snapshot execution retries 503 at most three times, report connection retries 502/503 at most three times, and temporary outages preserve the selected directory/job and surface a Persian in-page retry action instead of creating another job.
...
- Bale Mini App bootstrap preserves object-shaped capabilities and hidden drill-down routes, uses a 12-second abortable watchdog plus render boundaries, and emits rate-limited sanitized client telemetry to `/bale/mini-app/client-logs` after authentication.
- Bale Mini App resource queries are DTO-allowlisted by role/resource; path-scoped adviser IDs are not duplicated in query strings, and manager dashboard data must come from the bootstrap-provided Mini App endpoint rather than `/dashboard/stats`.
...
- Bale call start uses an in-app React confirmation modal, requires explicit call-room readiness, renders only masked contact numbers, and rotates the call idempotency key only after a definitive 502/invalid call-group response.
- Bale call-room questions are normalized at the service boundary from backend `question`/`options[].answer` fields; `multiChoice` controls radio versus checkbox rendering and numeric option IDs are preserved through draft restore and final submit.
- Bale connection administration supports permission-gated exceptional manual links through `/bale/admin/connections/manual`; submitted Bale IDs are normalized to 1–32 ASCII digits while remaining strings, and conflicts are never force-overridden.
